...
class KeyManager {
    ...
    public static function lockFederatedKey($file, $lock, $fs) {
        ...
        try {
            ...
            if (!empty($data["error"])) {
                $logger->error("Error " . $action . " federated key " . $data["error"], ["app" => self::App_Name]);
                return false;
            }
        } catch (\Exception $e) {
            $logger->logException($e, ["message" => "Failed to request federated " . $action, "app" => self::App_Name]);
            return false;
        }
    }
}
